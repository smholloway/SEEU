...
rule2 = Rule.create([{ 
  :id => 2,
  :rule => 'if ((Sensor.find(3).readings.first.data >= 0.to_s)) then a = Actuator.find(3).command; a.data = "low"; a.save; end', 
  :description => 'if the tiltometer is positive, turn the fan to low' 
}])
rule3 = Rule.create([{ 
  :id => 3,
